    ...
    <div class="container px-6 mx-auto grid">
        ...
        <template v-else-if="gamme">
            <h4 class="text-lg mt-4 font-semibold text-gray-600 dark:text-gray-300">
                ...
                >Restore</button>
                <button
                    v-else-if="gamme.status == 'pending' || gamme.status == 'rejected'"
                    @click="approve"
                    class="flex items-center float-right justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-primary-600 hover:bg-primary-700 border-2 border-primary-600 rounded-lg active:bg-primary-600 focus:outline-none focus:shadow-outline-primary"
                >Approve</button>
                <router-link
                    v-else
                    :to="{ name: 'gammes.edit', params: { slug: slug } }"
                    class="flex items-center float-right justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-primary-600 hover:bg-primary-700 border-2 border-primary-600 rounded-lg active:bg-primary-600 focus:outline-none focus:shadow-outline-primary">
                    <span>Edit</span>
                </router-link>
            </h4>
            ...
            <img v-if="gamme.image" :src="gamme.image" class="w-48 h-48 rounded-lg mb-5">
            
            <template v-if="gamme.description">
            ...
            <div class="grid gap-6 mb-8 md:grid-cols-2">
                ...
                <div v-if="gamme.discount_price" class="min-w-0 p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
                    <h4 class="mb-4 font-semibold text-gray-600 dark:text-gray-300">
                        Discount price:
                    </h4>
                    <p class="text-gray-600 dark:text-gray-400">
                        {{gamme.discount_price}} MAD
                    </p>
                </div>
            </div>
            ...
            <div class="grid gap-6 mb-8 md:grid-cols-2">
                <div v-for="product in gamme.products" :key="product.uuid" class="min-w-0 p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
                    <p class="text-gray-600 dark:text-gray-400">
                        {{product.name}}
                    </p>
                </div>
...
export default {
    ...
    methods: {
        ...
        restore() {
            this.$store.dispatch('restoreGamme', this.slug)
            .then(() => {
        ...
        approve() {
            this.$store.dispatch('approveGamme', this.slug)
            .then(() => {
                helpers.showAlert('success', 'Gamme approved!');
                this.loadGamme();
            })
            .catch((err) => {
                helpers.showAlert('error', err?.response?.statusText)
