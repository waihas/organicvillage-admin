...
export default {
    ...
    methods: {
            ...
            axios.get(`products/${this.slug}/show`)
                .then(res => {
                    if (res.data.data.status !== 'approved') {
                        return this.$router.push({ name: 'products.show', params: { slug: this.slug } })
                    }
        ...
        async update() {
            ...
            } else if (!this.prices[0].weight) {
                return this.errors.prices = "The weight is required.";
            } else if (!this.prices[0].price) {
            ...
            await this.$store.dispatch("updateProduct", { slug: this.slug, data: data })
                .then((isUpdated) => {
                    if(isUpdated) {
                        helpers.showAlert('success', 'Congratulations!', 'The product has been updated successfully.', false, 2000)
