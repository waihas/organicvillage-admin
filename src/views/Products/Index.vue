<template>
  <div class="container grid px-6 mx-auto">
    ...
    <h4 class="mb-4 text-lg font-semibold text-gray-600 dark:text-gray-300">
      List of products
      <router-link :to="{ name: 'products.create' }" class="flex items-center float-right justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-primary-600 border border-transparent rounded-lg active:bg-primary-600 hover:bg-primary-700 focus:outline-none focus:shadow-outline-primary">
      ...
      <div class="w-full overflow-x-auto">
        <table class="w-full whitespace-no-wrap">
          ...
          <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
            ...
            <tr v-for="product in products" :key="product.uuid" class="text-gray-700 dark:text-gray-400">
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3">
                <div class="flex items-center space-x-2 text-sm">
                  ...
                  <router-link
                    :to="{ name: 'products.edit', params: { slug: product.slug } }"
                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-primary-600 hover:text-primary-800 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                    aria-label="Edit"
                  >
...
export default {
  ...
  mounted() {
    this.loading = true;
    this.$store.dispatch('fetchProducts', this.page, this.perPage);
    this.loading = false;
  },
  data() {
    return {
  ...
  methods: {
    fetchProducts() {
      this.$store.dispatch('fetchProducts', { page: this.page, perPage: this.perPage, search: this.search });
    },
    updatedPage(page) {
      this.page = page;
      this.fetchProducts();
